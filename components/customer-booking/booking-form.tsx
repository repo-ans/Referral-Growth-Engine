...
import { getAvailableSlots, submitCustomerBooking } from "@/lib/actions/customer-booking";
...
// Public, no-account booking form — reached from a partner's /refer/[code]
// link. Unlike components/booking/booking-form.tsx (the authenticated
// version, still used by the untouched old /book flow), there's no profile
// to read name/phone/email from — every field is collected fresh here.
export function CustomerBookingForm({ code }: { code: string }) {
  const today = useMemo(() => {
    const d = new Date();
    d.setHours(0, 0, 0, 0);
    return d;
        ...
        <div className="mt-6">
          ...
          <div className="mt-3">
            ...
            <input
              ...
              className={inputClasses}
            />
            {step1Touched && phone.trim().length < 6 && (
